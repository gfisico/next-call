...
describe("POST /api/import/songs（取込）", () => {
  it("新規曲を upsert し、genres 複数・season/boolean/level を正しく変換する（基準1）", async () => {
    const csv = `${HEADER}
Stella By Starlight,Bb,AABA,Victor Young,1,1,1,0,1,通年,4,5,歌もの|バラード,
Recorda Me,C,OTHER,Joe Henderson,1,0,0,0,1,夏,3,3,ボサノバ,memo`;
    ...
    expect(stella.simpleForm).toBe(false);
...
Blue Bossa,C,OTHER,Kenny Dorham,0,0,0,0,0,通年,3,3,,`)).json();
    await commit(first.job.id);

    // 2回目: 同曲を正規化一致（前後空白・大小差）で更新
    const second = await upload(`${HEADER}
  BLUE  BOSSA ,Cm,AABA,Kenny Dorham,1,0,1,0,1,通年,5,4,ボサノバ,updated`);
  ...
  it("エラー行は行番号+理由付きで返り、有効行はプレビュー継続する（基準3）", async () => {
    const csv = `${HEADER}
Good Song,C,AABA,X,1,0,0,0,0,通年,3,3,歌もの,
Bad Boolean,C,AABA,X,9,0,0,0,0,通年,3,3,,
Bad Season,C,AABA,X,1,0,0,0,0,謎,3,3,,
Another Good,F,OTHER,Y,0,0,0,0,0,春,2,2,,`;
    const res = await upload(csv);
    const body = await res.json();
