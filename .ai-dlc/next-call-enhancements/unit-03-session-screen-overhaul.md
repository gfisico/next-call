---
status: completed
last_updated: "2026-07-16T04:56:02Z"
...
- API 呼び出しの body は camelCase（unit-01/unit-02 のAPIに合わせる。`sessionDate`/`venueId`/`instrumentCode`/`listenerCount`/`hostInstrumentCode`）。
- 要件1（履歴導線）・要件2（フロント編成カンマ表記）は API 非依存の純フロント変更。unit-02 完了を待たず**早期に先行コミット可能**（builder は依存の重い機能より先にこの2点を仕上げてよい）。ただし編集ファイルは本ユニットに閉じる。
