---
status: completed
last_updated: "2026-07-12T17:10:30Z"
...
3. **セッション記録画面（ACTIVE時の `/`）**:
   ...
   - **曲追加シート**（画面下部の Primary ボタン「曲を追加」）:
     ...
     - **共有コンポーネント契約**: 曲追加シートは初期値（song 固定、called_by_me、instrument 既定）と保存後コールバック/遷移先を props で受け取る共有コンポーネントとして実装する（unit-06 が「この曲をコール」から called_by_me=true・曲確定済みで再利用する）
...
## Success Criteria
- [x] セッション開始→曲追加（既存曲・クイック登録の両方）→ 編集・削除 → リスナートグル → 終了、の一連のフローがモバイルビューポート（375px）で操作できる（コンポーネント/E2E相当のテスト）
- [x] 新規店舗登録時のみ母店判定が表示され、既存店舗選択時は表示されない
- [x] 曲追加シートでフロント編成を vo, as, as, ts の順で登録でき、表示にも順序が反映される
- [x] 検索ヒットなし→クイック登録→そのまま演奏記録として追加、が一連で行える
- [x] POST 失敗時に入力値が消えず、リトライで成功する（fetch をモックしたテスト）
- [x] 「曲を追加」タップから保存まで、必須入力は曲名のみ（他は既定値）で完了できる（＝短時間登録）
- [x] 曲追加シートを「曲確定済み+called_by_me=true」の初期状態で開けるテストがある（unit-06 再利用契約）
- [x] 全画面が design_rule.md 準拠: Primary ボタンは画面内1つ、状態はバッジ+テキスト、focus-visible リング、コントラスト（deployable: 既存コンテナ構成のまま追加インフラなし）
