---
status: completed
last_updated: "2026-07-12T18:15:57Z"
...
## Success Criteria
- [x] defaults ロード→スライダー・チェック・編成・制約の変更→「候補を出す」→ 候補+理由表示、の一連が375pxで動作する（APIモックのテスト）
- [x] 前回意図値が引き継がれて表示され、変更した項目だけが変わる（仕様§9冒頭）
- [x] 1曲目のとき季節感が推奨ONで表示され、OFFに変更できる
- [x] 候補カードに推奨理由が2件以上表示される。isSparse 時に注記が出る
- [x] 条件別候補がラベル付きで区別して表示される（horns=UNKNOWN のモックデータ）
- [x] 保留曲枠が通常候補と独立して常時表示され、警告バッジ・コール・解除が機能する
- [x] 「この曲をコール」→ 曲追加シート（called_by_me=true・曲確定）→ 保存 → セッション画面へ戻る、が動作する
- [x] 「次の曲を考える」から候補表示まで最短2タップ（テストシナリオで確認。成功基準のタップ3回以内を満たす）
- [x] design_rule.md 準拠（スライダー・チップ・カード・バッジの実装は shadcn/ui ベース）（deployable: 追加インフラなし）
