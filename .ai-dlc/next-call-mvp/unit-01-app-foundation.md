...
- 環境変数: `DATABASE_PATH`, `AUTH_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `ALLOWED_EMAILS`（カンマ区切り許可メール）, `TZ`（`Asia/Tokyo` 固定）
- タイムゾーン規定: **日付の解釈は JST（Asia/Tokyo）を正とする**。`TZ=Asia/Tokyo` をコンテナ/開発環境に設定し（compose の environment と .env 一覧に含める。unit-09 と整合）、session_date 既定・季節判定・集計ウィンドウ等の「当日」の算出はこの TZ で行う
...
## Success Criteria
- [x] `npm run db:migrate && npm run db:seed` 後、全テーブルが作成され、ジャンルタグ9種・楽器12種・engine.* 設定が投入されている（テストで検証）
- [x] ALLOWED_EMAILS に含まれる Google アカウントでログインでき、含まれないアカウントはサインイン拒否される（signIn コールバックの単体テスト）
- [x] 未認証で任意のページ/APIにアクセスすると /login にリダイレクトされる（/api/health を除く）
- [x] app/globals.css に design_rule.md の必須トークンが Light/Dark 両方定義され、共通レイアウトが bg-background/text-foreground を使用している
- [x] GET /api/health が認証なしで 200 と DB 接続状態を返す（deployable/observable: コンテナのヘルスチェックと監視の基点）
- [x] typecheck / lint / test / build の4ゲートすべてがローカルでパスする
- [x] マイグレーションがアプリ起動時に自動適用される（deployable: 新規コンテナ起動だけでスキーマが最新化。operable: ロールバックは直前イメージへの切替のみで成立）
